...
[
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
        \Function{GeneriereBatches}{$A,b$}\Comment{A - float[][][], b - int (Batchgröße)}
            \State ${B} = Array[][][]$
            \For{$i = 0$ to $length(A)$}
...
\section{Auswertung der Ergebnisse}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
        \Function{EvaluatePrediction}{$P,b$}\Comment{P - int[][] (Ergebnis eines neuronalen Netzes), b - int (Batchgröße)}
            \State ${A} = int[]$
            \For{$i = 0$ to $length(P)$}
                \State $ClassesHit = int[]$\Comment{Hit-Punkte aller Klasse der Prediction für den aktuellen Zeitpunkt}
                \State $ClassesPerc = float[]$\Comment{Wahrscheinlichkeit aller Klasse der Prediction für den aktuellen Zeitpunkt}
                ...
                \State $A.append(EvaluateClassesOfPoint(ClassesHit, ClassesPerc))$

            \EndFor
            \State return $A$
        \EndFunction

        \Function{EvaluateClassesOfPoint}{$ClassesHit, ClassesPerc$}\Comment{ClassesHit - int[], ClassesPerc - float[]}
