...
\section{Vorbereiten der Daten}\label{VorbereitenDerDaten}
    ...
    Die Implementierung zu diesen Kapitel lässt sich in \url{https://github.com/schrodit/wesense-ml/blob/master/data/data_manager.py} finden.\\
...
\section{Neuronales Netz}
    ...
    \subsubsection{CNN}
    Grundlegend besteht das \ac{CNN} aus einem "`Input Layer"', drei "`Convolutional Layers"', einem "`Dropout Layer"' sowie einem "`Output Layer"'(vgl. \url{https://github.com/schrodit/wesense-ml/blob/master/classification/cnn_classification.py}).
    \newline
    ...
    \subsubsection{RNN}
    Das "`Recurrent Neural Network(RNN) "'- Model besteht aus einem "`Input Layer"', zwei darauffolgende "`LSTM(Long short-term memory) Layer"' und einem "`Ouput Layer"' (vgl. \url{https://github.com/schrodit/wesense-ml/blob/master/classification/rnn_classification_keras.py}).\\
    \noindent
    ...
    \noindent
    Da dieses Model eine Mischform aus \ac{CNN} und \ac{RNN} ist, besteht diese Model aus zwei "`Convolutional Layers"', welche die Eingabe und eine Vorprozessierung zum Generalisieren der Eingabe darstellt (vgl. \url{https://github.com/schrodit/wesense-ml/blob/master/classification/mix_classification.py}).
...
\section{Trainingsprozess}
    ...
    \section{Auswertungsalgorithmus}\label{Auswertungsalgorithmus}
        ...
        Der Vorteil dieses Verfahrens ist die Berücksichtigung aller Klassifikationen sowie deren einzel Wahrscheinlichkeit, sodass sich eine genauere Klassifizierung ergibt, wie Abbildung \ref{fig:CNN_RawClassification} zeigt.
